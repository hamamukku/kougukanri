...
BEGIN
    ...
    WITH projected AS (
        SELECT
            t.id,
            w.name || '-' || substring(t.asset_no FROM '([0-9]+)$') AS next_asset_no
        FROM tools t
...
CREATE TEMP TABLE tmp_tool_asset_no_000014_down
ON COMMIT DROP AS
SELECT
    t.id,
    w.name || '-' || substring(t.asset_no FROM '([0-9]+)$') AS next_asset_no
...
UPDATE tools
SET
    asset_no = '__tmp_asset_no_000014_down__' || id::text,
    updated_at = NOW();
...
SET
    ...
    updated_at = NOW()
FROM tmp_tool_asset_no_000014_down AS rewritten
